...
if st.button("Predict"):
    ...
    try:
        ...
        sepal_length = float(sepal_length.replace(",", "."))  # Replace comma with dot (optional)
        sepal_width = float(sepal_width.replace(",", "."))
        petal_length = float(petal_length.replace(",", "."))
        petal_width = float(petal_width.replace(",", "."))

        # Create model input list
        model_input = [sepal_length, sepal_width, petal_length, petal_width]

        # Make predictions using the loaded model
        prediction = model.predict([model_input])[0]

        # Map encoded prediction to original class labels
